...
OF2 = @(unknowns)ObjFun_BetaWithoutAge(t_actual2,params2,trajectories(jj+1,ll),options,priors,yinit2,unknowns);
unknowns = lsqnonlin(OF2,unknowns0,1E-3,10,options2);
priors = unknowns;
params2.beta= unknowns;
unknowns0 = unknowns;
Beta(jj+1) = unknowns;
[~,y2B] = ode45(@(t,y)seir_death_age_beta_b2(t,y,params2),t_actual(jj:jj+1),yinit2,options);
yinit2 = y2B(end,:);
yb2(jj+1,:) = yinit2;
end
